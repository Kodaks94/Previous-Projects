...
def step(state, action, trajectories_terminated, p_batch_size, corruption_to_physics_model=None):
    ...
    omega = tf.where( tf.abs(omega) > math.pi / 2, tf.cast(math.pi / 2,tf.float64), omega)
...
def dolearn2(iteration,start_states, final_artificial_gradient):
    with tf.GradientTape() as t:
        ...
        [total_reward, trajectory, action_hisotry,trajectories_terminated] = expand_trajectories(start_states,final_artificial_gradient, batch_size)
    ...
    return dCost_dWeights, dReward_dInputState, trajectory, total_reward,action_hisotry,trajectories_terminated
...
for iteration in range(max_iterations):
    ...
    gradients_list,d_reward_list, trajectory, total_reward, actions, trajectories_terminated = dolearn(iteration,state1,final_artificial_gradient)
    ...
    for i_ in range(1, pseudo_batch_size):
        if trajectories_terminated[i_ - 1]:
            ...
            if try_to_wrap_around_gradients:
                # the previous trajectory did crash, so it's final gradient should be zerofinal_artificial_gradient
            ...
            if try_to_wrap_around_gradients:
                # do trajectory wrap-around hack (backwards wrap-around of gradients)
                final_artificial_gradient[i_ - 1, :] = d_reward_list[i_]  # copy over the gradient information that came out of the start of the next trajectory, and feed it into the end of the current trajectory
    ...
    save_mat = np.concatenate([[action_history],[reward_history],[timestep_history]], axis=0)
    if save:
        if iteration % 10 == 0:
            np.save("runs/" + trial_name + "_marker_" + str(iteration) + "_results_" + filename + ".npy",save_mat)
            np.save("runs/" + trial_name + "_marker_" + str(iteration) + "_trajectory_history_" + filename + ".npy",np.array(trajectory_history))
            action_history = []
            reward_history = []
            timestep_history = []
            trajectory_history = []
