...
initial_state = reset()
#step(state, action, trajectories_terminated, batch_size)
...
action_list = []
@tf.function
def expand_trajectories(start_states):
    total_rewards = tf.constant(0.0, shape=[batch_size])
    recorded_total_rewards = tf.constant(0.0, shape= [batch_size])
    actions = tf.zeros((batch_size, action_space))
    action_list = []
    trajectory=tf.expand_dims(start_states,axis=0)
    trajectories_terminated = tf.cast(tf.zeros_like(start_states[:, 0]), tf.bool)
    state = start_states
    print( tf.TensorShape([None, state.get_shape()[0],state_dimension]))
    print(np.shape(recorded_total_rewards))
    [state,total_rewards,recorded_total_rewards,trajectory,trajectories_terminated]=\
        tf.while_loop(while_loop_cond, while_loop_body, (state,total_rewards,recorded_total_rewards,trajectory,trajectories_terminated),
                      shape_invariants=(state.get_shape(),
                                        total_rewards.get_shape(),
                                        recorded_total_rewards.get_shape(),
    ...
    trajectory = tf.stack(trajectory, axis=0)
...
@tf.function
def while_loop_cond(state,total_rewards,recorded_total_rewards,trajectory,trajectories_terminated):
    return tf.logical_not(tf.reduce_all(trajectories_terminated))
@tf.function
def while_loop_body(state,total_rewards,recorded_total_rewards,trajectory,trajectories_terminated):
    converted_state = converter(state, batch_size)
    prevaction = keras_action_network(converted_state)
    action = tf.reshape(prevaction, (batch_size, action_space))
    [rewards, recorded_tanh, n_state, trajectories_terminating] = step(state, action, trajectories_terminated,
                                                                       batch_size)
    ...
    recorded_tanh = tf.reshape(recorded_tanh, (batch_size,))
    ...
    recorded_total_rewards += tf.where(
        tf.logical_and(trajectories_terminating, tf.logical_not(trajectories_terminated)),
    ...
    return state,total_rewards,recorded_total_rewards,trajectory,trajectories_terminated

@tf.function
def dolearn(start_states):
    with tf.GradientTape() as t:
        t.watch(start_states)
        [total_reward,recorded_total_reward, trajectory, action_hisotry,trajectories_terminated] = expand_trajectories(start_states)
        cost_ = -tf.reduce_mean(total_reward)
    grads = t.gradient(cost_, keras_action_network.trainable_weights)
    return grads, trajectory, total_reward,recorded_total_reward,action_hisotry,trajectories_terminated
...
reward_history = []
trajectory_history = []
initial_state_backup = initial_state
...
for iteration in range(max_iterations):
    state1 = initial_state_backup
    # trajectories_terminated = trajectories_terminated
    gradients_list, trajectory, total_reward,rc_reward, actions, trajectories_terminated = dolearn(state1)
    ...
    average_total_reward_stepwise = np.max(total_reward.numpy())
    rc_average_total_reward_stepwise = np.max(rc_reward.numpy())
    reward_history.append(rc_average_total_reward_stepwise)
    ...
    if iteration % print_time == 0:
        ...
        if prinit:
            t_b = datetime.now()
            dt = t_b - t_a
            print("iteration: ", iteration, "// Average_total_reward_step_wise: ", rc_average_total_reward_stepwise,
    ...
    if save:
        if iteration % 10 == 0:
            ...
            np.save("withoutClipping/" + trial_name + "_marker_" + str(iteration) + "_results_" + filename + ".npy",save_mat)
            #np.save("runs5/" + trial_name + "_marker_" + str(iteration) + "_action_history_" + filename + ".npy",np.array(action_history))
            #np.save("runs5/" + trial_name + "_marker_" + str(iteration) + "_trajectory_history_" + filename + ".npy",np.array(trajectory_history))
            np.save("withoutClipping/last_state_" + filename + ".npy", trajectory)
            ...
            trajectory_history = []
        if iteration % 10 == 0:
            keras_action_network.save_weights("./checkpoints/my_checkpoint"+trial_name)
if save:
    np.save("withoutClipping/" + trial_name + "_marker_" + str(iteration) + "_results_" + filename + ".npy",save_mat)
    #np.save("withoutClipping/" + trial_name + "_marker_" + str(iteration) + "_trajectory_history_" + filename + ".npy",np.array(trajectory_history))
    np.save("withoutClipping/last_state_" + filename + ".npy", trajectory)
    keras_action_network.save_weights("./checkpoints/my_checkpoint_"+trial_name)